...
\claude{Section~\ref{sec:inference_acquisition} now defines the criterion, and says plainly that neither term of \eqref{eq:eig} is a closed form: the predictive entropy is a truncated support sum and the aleatoric term a quadrature. The abstract still says ``available in closed form'' and the conclusion repeats it. Those two sentences now contradict the body and need rewording to ``deterministic, free of Monte-Carlo error and of nesting bias''. Same paragraph carries the cost claim that Table~\ref{tab:bulk} contradicts: D-optimality matches our accuracy at $0.5$\,ms against our $8.8$.}
...
\subsection{Scoring a Candidate Input}
\label{sec:inference_acquisition}

The sensor scores a candidate by the information the count it returns is expected to carry about the rate \citep{lindley1956measure,houlsby2011bald}, that is by the mutual information between $\lambda$ and $y$ under the current belief,
%
\begin{equation}
    \begin{split}
        G(u) &= \Ex{p(y \given u, \mathcal{D})}{ D_{\text{KL}}\big[ p(\lambda \given y, u, \mathcal{D}) \,\|\, p(\lambda \given \mathcal{D}) \big] } \\
        &= H\big[ p(y \given u, \mathcal{D}) \big] - A(u) \,,
    \end{split}
    \label{eq:eig}
\end{equation}
%
where the second line is the standard rearrangement of the expected Kullback-Leibler divergence into a predictive entropy less an expected conditional entropy,
%
\begin{equation}
    A(u) = \Ex{p(\lambda \given \mathcal{D})}{ H\big[ \mathcal{P}\big(y \given f(u) \lambda \big) \big] } \,.
    \label{eq:aleatoric}
\end{equation}

\begin{proposition}[Two deterministic quadratures]
    \label{prop:eig}
    Under \eqref{eq:likelihood} and \eqref{eq:gig}, both terms of \eqref{eq:eig} are integrals against known densities and neither requires a sample.
    The first is a sum of \eqref{eq:sichel} over the support of the predictive; the second is a one-dimensional integral of the Poisson entropy against the belief \eqref{eq:gig}.
\end{proposition}

Neither term is a closed form.
The support of \eqref{eq:sichel} is unbounded, so $H[p(y \given u, \mathcal{D})]$ is a sum truncated at a fixed tail tolerance, and the Poisson entropy inside \eqref{eq:aleatoric} has no elementary expression, so $A(u)$ is a quadrature.
What Proposition~\ref{prop:eig} does buy is that both are \emph{deterministic}.
The value of \eqref{eq:eig} carries no Monte-Carlo error, has no sample size to tune, and in particular no nesting bias, which is what an estimator of the same quantity outside a conjugate family has to accept \citep{rainforth2018nesting,rainforth2024modern}.
We report the cost of that difference in Section~\ref{sec:experiments_bulk}.

Two things make the evaluation cheap enough to put inside a loop over candidates.
The first is Proposition~\ref{prop:predictive}: every dependence of both terms on the input passes through the scalar $f(u)$, so the belief-level quantities are computed once and a grid of candidates costs one pass apiece with nothing re-solved.
The second is that \eqref{eq:aleatoric} is an integral over a single positive variable, which is the whole of the parameter addressed by the input.
In our implementation the support sum of the first term is truncated at a tail mass of $10^{-13}$, sized from the predictive moments of \eqref{eq:predictive_mean} and \eqref{eq:predictive_variance} and widened until the tolerance is met, since a normal-tail estimate is not conservative for a heavy-tailed law; the second is a $64$-node Gauss-Legendre rule in $\log \lambda$, over the bracket on which the log-density of \eqref{eq:gig} has fallen $75$ nats from its mode.
Both are accumulated in logarithms, for the reason Remark~\ref{rem:conditioning} gives.

The decomposition in \eqref{eq:eig} is also what separates the criteria compared in Section~\ref{sec:experiments}.
A criterion that scores an input by the predictive entropy alone, as maximum entropy sampling does \citep{sebastiani2000maximum}, is \eqref{eq:eig} with $A(u)$ dropped.
By \eqref{eq:fano} the aleatoric term grows with the mean of the count, so dropping it rewards an input for the noise it collects rather than for the uncertainty it removes, and Section~\ref{sec:experiments_bulk} shows what that costs.
...
Here the rates are the photon fluxes of $5065$ sources in the Fermi Large Area Telescope fourth source catalogue \citep{abdollahi2020fermi}, and the instrument that measured them is a photon counter, so the counting model of Section~\ref{sec:model} is the measurement process rather than an analogy.
...
\onecolumn
\aistatstitle{Actively sensing overdispersed count data\\ with Sichel distributed predictions:\\ Supplementary Materials}
